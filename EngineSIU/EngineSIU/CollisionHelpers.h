...
{
    // 박스의 로컬 축 (월드 공간 기준)
    FVector AxisX = FMatrix::GetColumn(BoxRotMat, 0);
    FVector AxisY = FMatrix::GetColumn(BoxRotMat, 1);
    FVector AxisZ = FMatrix::GetColumn(BoxRotMat, 2);
...
//FVector ClosestPointOnOBB(const FVector& Point,const FVector& BoxCenter, const FVector& BoxRotation, const FVector& BoxExtent)
//{
